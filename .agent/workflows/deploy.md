...
> **💡 自己Ping機能とスケジュール同期について**
> 
> バックエンドには自己Ping機能が組み込まれており、14分ごとに自分自身の `/health` エンドポイントにアクセスします。
> また、`/api/sync-schedule` エンドポイントは UptimeRobot 等で定期的に叩くことで、TimeTreeの予定を自動同期します。
> 内部で「変更があった場合のみAI解析を実行する」最適化が行われているため、高頻度（5分おき等）で叩いてもAIコストを最小限に抑えつつリアルタイムな同期が可能です。
...
| **URL**                 | `https://ai-mau-bot-backend.onrender.com/api/sync-schedule?token=<あなたのトークン>` |
