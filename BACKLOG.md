...
| 優先度 | 機能名 | 詳細・目的 | ステータス |
| :--- | :--- | :--- | :--- |
| **高** | **テスト環境構築** | **【品質】** ぶっつけ本番をやめる。<br>・Dev用ボットを作成し、ローカル`.env`をDev用トークンに切り替え。<br>・ローカルで実験 → GitHub経由で本番反映のフローを確立する。 | ☁️ 未着手 |
| **高** | **API認証 (Secret Key)** | **【防衛】** 公開APIを他人に勝手に使われないようにする。<br>・ヘッダー認証(`MAU_API_SECRET`)を実装する。 | ☁️ 未着手 |
| **高** | **管理者IDの秘匿** | **【防衛】** ソースコードに個人IDを書かない。<br>・`ADMIN_USER_ID` を環境変数から読み込むように修正。 | ☁️ 未着手 |
...
| 優先度 | 機能名 | 詳細・目的 | ステータス |
| :--- | :--- | :--- | :--- |
| **中** | **管理者アナウンス** | **【運営】** `!announce` コマンドで、まうちゃんが代理告知ツイートを行う。<br>・セキュリティ対策とセットで実装。 | ☁️ 未着手 |
