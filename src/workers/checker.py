...
# ターゲット: ろりぽっぷ!!!!!!! 公開カレンダー
TARGET_URL: str = "https://timetreeapp.com/public_calendars/lollipop_1116"

def check_timetree() -> None:
    logger.info(f"🔄 アクセス中: {TARGET_URL} ...")
    ...
    try:
        ...
        logger.info(f"✅ アクセス成功 (Status: {res.status_code})")
        ...
        if not script_tag:
            logger.error("❌ エラー: '__NEXT_DATA__' タグが見つかりませんでした。サイト構造が変わった可能性があります。")
            return

        logger.info("✅ JSONデータ発見！解析します...")
        ...
        try:
            ...
            # デバッグ用にキーを表示
            # logger.debug(f"Debug Keys: {base_data.keys()}")
            ...
            if not events:
                logger.warning("⚠️ イベントリストが空、またはパスが違います。")
                # 念のため raw data の一部を表示
                logger.debug(f"Top Level Keys: {data.keys()}")
                return

            logger.info(f"🎉 取得成功！ {len(events)} 件のイベントが見つかりました。")
            logger.info("--- 直近のイベント ---")
            ...
            for i, event in enumerate(events[:5]): # 最初の5件だけ表示
                ...
                logger.info(f"[{i+1}] {date_str} : {title}")
